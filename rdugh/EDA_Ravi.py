...
from pyspark.sql.functions import *
raviairDF_2['DEP_HOUR']=raviairDF_2.withColumn('DEP_TIME',col('DEP_TIME').cast(str))
     
raviairDF_2['DEP_HOUR']=raviairDF_2.select(col('DEP_HOUR'),to_timestamp(col('DEP_HOUR'),"HHmm" ))

# COMMAND ----------

spark.sql("USE {}".format(databaseName))
outputPathSilver= BASE_DELTA_PATH+"ravi_silver_table"
sqlCmd = """
    CREATE TABLE IF NOT EXISTS ravi_silver_table
    USING DELTA
    
    LOCATION "{}"
    AS 
    SELECT ORIGIN_AIRPORT_ID, DEST_AIRPORT_ID, FL_DATE, DEP_TIME, DEP_DELAY, ARR_DELAY, ORIGIN_CITY_NAME, DEST_CITY_NAME, ORIGIN, DEST, DAY_OF_WEEK, MONTH, YEAR, QUARTER, DAY_OF_MONTH, OP_UNIQUE_CARRIER,CANCELLED,CANCELLATION_CODE,AIR_TIME,DISTANCE,DISTANCE_GROUP,CARRIER_DELAY,WEATHER_DELAY, raviweatherDF_3.call_sign, raviweatherDF_3.avg_temp_f,raviweatherDF_3.tot_precip_mm, raviweatherDF_3.avg_wnd_mps,raviweatherDF_3.avg_slp_hpa,raviweatherDF_3.avg_dewpt_f,raviweatherDF_3.year,raviweatherDF_3.month, raviweatherDF_3.dayofmonth,raviweatherDF_3.avg_dewpt_f.hour
    FROM raviweatherDF_3, raviairDF_2
    INNER JOIN raviairDF_2 ON raviairDF_2.YEAR = raviweatherDF_3.year
    INNER JOIN raviairDF_2 ON raviairDF_2.MONTH = raviweatherDF_3.month
    INNER JOIN raviairDF_2 ON raviairDF_2.DAY_OF_MONTH = raviweatherDF_3.dayofmonth
    INNER JOIN raviairDF_2 ON raviairDF_2.DEP_TIME=raviweatherDF_3.hour
